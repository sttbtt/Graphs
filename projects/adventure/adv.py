...
while len(graph) < len(roomGraph):
    ...
    if player.currentRoom.id not in graph:
        # Initialize in your room graph with '?'
        graph[player.currentRoom.id] = {}
        for room_exit in player.currentRoom.getExits():
            graph[player.currentRoom.id][room_exit] = '?'
...
# player.currentRoom.id = 16

print(f'Traversal Path: {traversalPath}')
# print(f'Ending Graph: {graph}')
print(f'Length of Ending Graph: {len(graph)}')
print(f'Length of Traversal Path: {len(traversalPath)}')
rooms_visited = []
...
#     graph[player.currentRoom.id] = {}
#     for ex in exits:
#         graph[player.currentRoom.id][ex] = '?'
#     if previousRoom is not None:
#         graph[previousRoom][dirToMove] = player.currentRoom.id
#         oppositeDirection = inverseDirections[dirToMove]
#         graph[player.currentRoom.id][oppositeDirection] = previousRoom
...
# info = {player.currentRoom.id: player.currentRoom.getExits()}
# print(graph)
# graph.append(info)

# player.travel('n')
# graph.add(player.currentRoom.id)
# player.currentRoom.printRoomDescription(player)
# print(player.currentRoom.getExits())
...
# add exits to graph{}
# while there is an exit move to next room
# if there isn't an exit backtrack
